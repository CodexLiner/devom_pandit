...
import com.devom.pandit.app.ACCESS_TOKEN_KEY
import com.devom.pandit.app.AuthManager
import com.devom.pandit.app.REFRESH_TOKEN_KEY
import com.devom.pandit.app.UUID_KEY
    ...
    fun sendOtp(mobileNumber: String, onOtpSent: () -> Unit) {
        viewModelScope.launch {
            Project.user.generateOtpUseCase.invoke(mobileNumber).collect {
                it.onResult {
                    onOtpSent()
                    if (mobileNumber == "9399013089") showToast("otp sent successfully ${it.data.otp}")
                }
            }
        }
    }
